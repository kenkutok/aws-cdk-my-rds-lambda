...
export class AwsCdkMyRdsLambdaStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    // Create the Serverless Aurora DB cluster, and set the engine to Postgres
    const rdsAurora = new rds.ServerlessCluster(this, 'AuroraCluster', {
    ...
    const postFn = new lambda.Function(this, 'MyFunction', {
      ...
      memorySize: 1024,
      environment: {
        CLUSTER_ARN: rdsAurora.clusterArn,
        SECRET_ARN: rdsAurora.secret?.secretArn || '',
        DB_NAME: 'TestDB'
      },
    });

    // Grant access to the cluster from the Lambda function
    rdsAurora.grantDataApiAccess(postFn);
